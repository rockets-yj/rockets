...
def userHosting(request):
    ...
    if request.method == 'POST' :
        ...
        # 중복값 확인 아래 추가하고 if문, else문 적용하기
        if Serviceaws.objects.filter(service_name=_serviceName).exists():
            error_message = '이미 사용중인 서비스 이름입니다.'
            return render(request, 'hosting/hostingPage.html', {'error': error_message})
